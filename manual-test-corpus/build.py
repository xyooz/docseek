...
from tools.wps_fixtures import load_manifest, materialize_fixture
...
WPS_OLE_FIXTURES: dict[str, dict[str, Any]] = load_manifest()
...
def reconstruct_wps_fixture(name: str, target: Path) -> None:
    # Keep corpus generation on the same integrity-checked path as tests and
    # packaged smoke helpers. A malformed transport payload must fail before
    # it reaches the generated acceptance corpus.
    materialize_fixture(name, target, manifest=WPS_OLE_FIXTURES)
...
- `05-wps/`：只包含 manifest 中通过完整性校验的三个真实/脱敏 WPS OLE 样例。
...
`05-wps/` 只包含 manifest 中通过 size、SHA-256 和 OLE/CFB magic 校验的三个样例。
"""
