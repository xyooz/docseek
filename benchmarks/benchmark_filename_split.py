...
def _payload(index: int, chunk_no: int, payload_bytes: int) -> str:
    ...
    if index % 4 == 0 and chunk_no == 0:
        markers.append("业务流程")
    if index % 20 == 0 and chunk_no == 1:
        markers.append("客户经理 信贷政策")
    if index % 50 == 0 and chunk_no == 2:
        markers.append("跨境专项复核")
    if index % 10 == 0 and chunk_no == 0:
        # Paired with a filename-only marker to exercise cross-source AND.
        markers.append("风险整改")

    base = " ".join(markers) + f" 文档 {index} 内容块 {chunk_no} 银行业务资料 "
...
def _create_repeated_schema(conn: sqlite3.Connection) -> None:
...
def _create_split_schema(conn: sqlite3.Connection) -> None:
    _create_common_tables(conn)
    conn.executescript(
        """
...
def _build(
    ...
    *,
    ...
    _configure(conn)
    (_create_split_schema if split else _create_repeated_schema)(conn)
    ...
    for index in range(files):
        ...
        file_id = int(
            conn.execute("INSERT INTO files(filename) VALUES (?)", (filename,)).lastrowid
        )
        if split:
            conn.execute(
            ...
            conn.execute(
                ...
                (file_id, ChunkStore._cjk_bigrams(filename)),
            )
        ...
        for ordinal in range(chunks_per_file):
            ...
            if split:
                ...
                conn.execute(
                    ...
                    (chunk_id, ChunkStore._cjk_bigrams(content)),
                )
            else:
                conn.execute(
                ...
                conn.execute(
                    ...
                    (
                        chunk_id,
                        ChunkStore._cjk_bigrams(filename),
...
def _cjk_phrase(text: str) -> str:
    tokens = ChunkStore._cjk_bigrams(text).split()
    return '"' + " ".join(tokens) + '"'


def _repeated_count(conn: sqlite3.Connection, query: str) -> int:
...
def _split_single_source_count(
...
def _split_cross_source_count(
    ...
    filename_phrase = _cjk_phrase(filename_term)
    content_phrase = _cjk_phrase(content_term)
    row = conn.execute(
        """
        ...
        """,
        {
            "filename_query": filename_phrase,
            "content_query": content_phrase,
        },
    ).fetchone()
    return int(row[0])
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Compare repeated per-chunk filename FTS with a split file/content index"
    )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        _configure(split)
        try:
            filename_old = _measure(
                lambda: _repeated_count(repeated, "专项检查"),
                iterations=args.iterations,
            )
            filename_new = _measure(
                lambda: _split_single_source_count(split, "专项检查", filename=True),
                iterations=args.iterations,
            )
            content_old = _measure(
                lambda: _repeated_count(repeated, "跨境专项复核"),
                iterations=args.iterations,
            )
            content_new = _measure(
                lambda: _split_single_source_count(split, "跨境专项复核", filename=False),
                iterations=args.iterations,
            )
            cross_old = _measure(
                lambda: _repeated_count(repeated, "专项检查 风险整改"),
                iterations=args.iterations,
            )
            cross_new = _measure(
        ...
        print(
            f"repeated: index={repeated_time:.3f}s db={repeated_bytes / 1024 / 1024:.2f}MiB"
        )
        print(
            f"split:    index={split_time:.3f}s db={split_bytes / 1024 / 1024:.2f}MiB"
        )
        print(
        ...
        for label, old, new in (
            ("filename-only", filename_old, filename_new),
            ("content-only", content_old, content_new),
            ("cross-source-and", cross_old, cross_new),
        ):
            old_p50, old_p95, old_count = old
            new_p50, new_p95, new_count = new
            print(
                f"{label:16s} repeated(p50={old_p50:.2f}, p95={old_p95:.2f}, n={old_count})ms "
