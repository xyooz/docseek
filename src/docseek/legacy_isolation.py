...
def available_legacy_adapter_names(source: Path) -> tuple[str, ...]:
    ...
    for adapter in DEFAULT_ADAPTER_REGISTRY.candidates_for(source):
        try:
            ...
            # Availability probes are not allowed to break indexing. A failing
            # vendor probe simply removes that backend from this attempt.
            continue
    return tuple(names)
...
def iter_legacy_chunks_isolated(
    ...
    *,
    ...
    Every backend runs in its own killable process. Calamine/Tika/WPS failures
    therefore remain isolated, and one backend error does not prevent trying the
    next mature local parser. ``timeout_seconds`` is the total per-file budget;
    ``adapter_timeout_seconds`` prevents one backend from consuming it all.
    """
    source = Path(source)
