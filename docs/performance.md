...
Grouped minima 可以避免 `ROW_NUMBER() OVER(PARTITION BY ...)` 的全量窗口排序。在 10k 广泛查询中有约 1.1x～1.4x 的收益记录，但 1k 上收益不稳定，部分 run 会略慢。
...
10k 广泛查询中常驻连接仅比新连接快约 5%～10%，说明规模瓶颈已经转移到精确聚合/排序，而不是连接初始化或 page cache。

## 后续优先级

1. 增加更接近真实办公文档的混合选择率 workload，避免只用“所有文件都命中”的极端合成数据判断架构。
2. 研究可证明正确的 rank-safe Top-K，而不是固定候选截断。
3. 将文件名检索与 Chunk 正文检索进一步解耦，避免文件名在每个 Chunk FTS 行中重复。
4. 若 10k/50k 真实 workload 仍无法满足交互延迟，再评估 Tantivy 等专用倒排后端；提取器、Chunk 模型、文件元数据层保持独立，避免绑定具体检索引擎。

## 运行大规模 benchmark

大规模 benchmark 已从普通 push CI 移除。可在 GitHub Actions 中手动运行 `benchmarks` workflow，并设置文件数、每文件 Chunk 数、Chunk 大小和迭代次数。
